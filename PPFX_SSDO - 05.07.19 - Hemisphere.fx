...
#ifndef pSSDOLOD
#define		pSSDOLOD					1.0		// SSDO LOD - The resolution which the effect is calculated in. 1 = full-screen, 2 = half, 4 = quarter, 8 = one-eighth
#endif
...
// *** EXTERNAL TEXTURES ***
texture texNoise < source = "bayer4x4ppfx.png"; >
{
	Width = 4;
	Height = 4;
	Format = R8;
...
// *** SSDO ***
	...
	#define SSDO_BLUR_DEPTH_DISCONTINUITY_THRESH_MULTIPLIER 0.03
	...
	float4 FX_SSDOScatter( float2 txCoords )
	{
		...
		for (float offs=1.0;offs<=pSSDOSampleAmount;offs++)
		{
			...
			fetchDir *= sign(dot(float2(fetchDir.x,-fetchDir.y),normalize(vsOrig.xy))); // sample in a hemisphere
			float2 fetchCoords = txCoords+fetchDir*offs*stepSize;
			...
			float visibility = step(0.05,dot(dirVecN,vsOrig.xyz)); // visibility check w/ angle threshold
		...
		return float4(saturate(1.0-ssdo*smoothstep(0.1,0.2,1.0-vsOrig.w)),vsOrig.w);
	}
