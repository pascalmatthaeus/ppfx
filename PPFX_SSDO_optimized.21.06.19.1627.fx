...
// *** EXTERNAL TEXTURES ***
texture texNoise < source = "noise.png"; >
{
	Width = 12;
	Height = 12;
	#define NOISE_SCREENSCALE float2((BUFFER_WIDTH/pow(2.0,pSSDOLOD))/12.0,(BUFFER_HEIGHT/pow(2.0,pSSDOLOD))/12.0)
};
...
// *** SSDO ***
	...
	#define SSDO_BLUR_DEPTH_DISCONTINUITY_THRESH_MULTIPLIER 0.08
	...
	float4 FX_SSDOScatter( float2 txCoords )
	{
		...
		float2	randomDir = normalize(tex2D(SamplerNoise,frac(txCoords*NOISE_SCREENSCALE)).xy);
		...
		{
			float2 fetchCoords = txCoords+normalize(frac(randomDir*11.139795*offs)*2.0-1.0)*offs*stepSize;
			...
			float visibility = max(0.0,dot(dirVecN,vsOrig.xyz)-0.05)/0.95;
			float distFade = max(0.0,SSDO_CONTRIB_RANGE-length(dirVec))/SSDO_CONTRIB_RANGE;
			ssdo += albedoFetch * visibility * distFade;
		}
		ssdo /= pSSDOSampleAmount;
		
		return float4(saturate(1.0-ssdo*smoothstep(0.01,0.2,1.0-vsOrig.w)),vsOrig.w);
	}
...
// *** SSDO ***
    ...
    float3 SSDO_MIX_MODE;
      ...
      return float4(ssdo * tex2D(ReShade::BackBuffer,IN.txcoord.xy).xyz,1.0);
	}
